...
public class DataTableRenderer extends CoreRenderer{
	...
	public void encodeEnd(FacesContext facesContext, UIComponent component) throws IOException{
		...
		encodeDataTableWidget(facesContext, dataTable);
		encodeDataTableMarkup(facesContext, dataTable);
	}
	
	private void encodeDataTableWidget(FacesContext facesContext, DataTable dataTable) throws IOException{
		ResponseWriter writer = facesContext.getResponseWriter();
		
		String dataTableVar = getDataTableVar(dataTable);
	...
	private void encodeRowsMarkup(FacesContext facesContext, DataTable dataTable) throws IOException {
		...
		for (int i = 0; i < rowCount; i++) {
			...
			facesContext.getExternalContext().getRequestMap().put(dataTable.getVar(), dataTable.getRowData());
		...
		dataTable.setRowIndex(rowIndex);		//reset
		
		writer.endElement("tbody");
	}

	private String getDataTableVar(DataTable dataTable) {
		if(dataTable.getWidgetVar() != null)
			return dataTable.getWidgetVar();
		else
			return "pf_dataTable_" + dataTable.getId();
	}
