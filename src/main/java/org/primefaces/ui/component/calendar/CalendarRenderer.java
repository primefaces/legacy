...
public class CalendarRenderer extends CoreRenderer{
	...
	private static String DEFAULT_PATTERN = "MM/dd/yyyy";
	private static String DEFAULT_POPUP_ICON = "/yuiext/calendar/calendar_icon.png";
	...
	private void encodeCalendarScript(FacesContext facesContext, Calendar calendar) throws IOException{
		...
		String clientId = calendar.getClientId(facesContext);
		String calendarVar = getCalendarVar(calendar);
		...
		if(!isPopup(calendar))
			...
			writer.write("YAHOO.util.Event.addListener(\"" + clientId + ":popupButtonImage\", \"click\", function(e){" + calendarVar + ".render();" + calendarVar +"_popupOverlay.show();});");
	...
	protected String getCalendarVar(Calendar calendar) {
		if(calendar.getWidgetVar() != null)
			return calendar.getWidgetVar();
		else
			return "pf_calendar_" + calendar.getId();
	}
