...
public class LineChartRenderer extends BaseChartRenderer implements PartialRenderer {
	...
	private void encodeChartWidget(FacesContext facesContext, UIChart chart, String clientId, String xfieldName) throws IOException {
		...
		if(shouldRenderAttribute(lineChart.getMinY()) || shouldRenderAttribute(lineChart.getMaxY())) {
			writer.write("var axis = new YAHOO.widget.NumericAxis();\n");
			if(shouldRenderAttribute(lineChart.getMinY()))
				writer.write("axis.minimum = " + lineChart.getMinY() + ";\n");
			if(shouldRenderAttribute(lineChart.getMaxY()))
				writer.write("axis.maximum = " + lineChart.getMaxY() + ";\n");
