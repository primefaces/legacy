...
public class ButtonRenderer extends CoreRenderer {
	...
	protected void encodeMarkup(FacesContext facesContext, Button button) throws IOException {
		...
		if(confirmDialog != null) {
			encodeButtonMarkup(facesContext, clientId + ":proxy", "button", (String) button.getValue());	//proxy button
			encodeConfirmDialogMarkup(facesContext, button, confirmDialog);
		} else {
			encodeButtonMarkup(facesContext, clientId + ":button", resolveType(button), (String) button.getValue());
		}
	...
	protected void encodeConfirmDialogMarkup(FacesContext facesContext, Button button, ConfirmDialog confirmDialog) throws IOException {
		...
		encodeButtonMarkup(facesContext, clientId + ":button", resolveType(button), confirmDialog.getYesLabel());
		encodeButtonMarkup(facesContext, clientId + ":noButton", "button", confirmDialog.getNoLabel());
	...
	protected void encodeButtonMarkup(FacesContext facesContext, String id, String type, String label) throws IOException {
		ResponseWriter writer = facesContext.getResponseWriter();
	...
	protected void encodeScript(FacesContext facesContext, Button button) throws IOException {
		...
		if(confirmDialog != null) {
			...
			writer.write(confirmDialogVar + " = new YAHOO.widget.Panel(\"" + clientId + ":confirmationContainer\", " + confirmDialogCFGVar + ");\n");
			writer.write(confirmDialogVar + ".render();");
			
			String proxyButtonVar = buttonVar + "_proxy";
			writer.write("var " + proxyButtonVar + "  = new YAHOO.widget.Button(\""+ clientId + ":proxy\");\n");
			writer.write(proxyButtonVar + ".addListener(\"click\", function(e){" + confirmDialogVar + ".show();});\n");
			
			String noButtonVar = buttonVar + "_no";
			writer.write("var " + noButtonVar + "  = new YAHOO.widget.Button(\""+ clientId + ":noButton\");\n");
			writer.write(noButtonVar + ".addListener(\"click\", function(e){" + confirmDialogVar + ".hide();});\n");
		}
		
		writer.write(buttonVar + " = new YAHOO.widget.Button(\""+ clientId + ":button\");\n");
	...
	private void renderDomEvents(FacesContext facesContext, Button button, String buttonVar) throws IOException {
		ResponseWriter writer = facesContext.getResponseWriter();
