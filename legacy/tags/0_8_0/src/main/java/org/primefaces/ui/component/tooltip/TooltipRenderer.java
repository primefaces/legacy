...
public class TooltipRenderer extends CoreRenderer{
	...
	private void encodeTooltipWidget(FacesContext facesContext, Tooltip tooltip) throws IOException{
		...
		UIComponent forComponent = ComponentUtils.findComponentById(facesContext, facesContext.getViewRoot(), tooltip.getFor());
		if(forComponent == null)
			throw new IllegalArgumentException("Cannot find component id:" + tooltip.getFor() + " in view");
		...
		String tooltipVar = getTooltipVar(tooltip);
		
		writer.startElement("script", null);
		writer.writeAttribute("type", "text/javascript", null);
		
		writer.write("YAHOO.util.Event.onContentReady(\"" + forComponentClientId + "\", function() {\n");
		writer.write(tooltipVar + " = new YAHOO.widget.Tooltip(\"" + clientId + ":container\",");
		writer.write("{context:\"" + forComponentClientId + "\", text:\"" + value  + "\"");
	...
	private String getTooltipVar(Tooltip tooltip) {
		if(tooltip.getWidgetVar() != null)
			return tooltip.getWidgetVar();
		else
			return "pf_tooltip_" + tooltip.getId();
	}
}