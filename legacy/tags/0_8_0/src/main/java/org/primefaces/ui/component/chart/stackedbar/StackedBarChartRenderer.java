 ...
 */
package org.primefaces.ui.component.chart.stackedbar;
...
public class StackedBarChartRenderer extends CoreRenderer{
	...
	private void encodeChartWidget(FacesContext facesContext, StackedBarChart chart) throws IOException{
		...
		writer.write(chartVariable + "_responseSchema = {fields:[\"" + yfieldName + "\"");
		for (ChartSeries axis : series) {
		...
		writer.write(chartVariable + " = new YAHOO.widget.StackedBarChart(\"" + chart.getClientId(facesContext) + "\"," + chartVariable + "_dataSource,{");
	...
	protected List<ChartSeries> getSeries(StackedBarChart chart) {
	...
	protected void encodeChartMarkup(FacesContext facesContext, StackedBarChart chart) throws IOException{
		ResponseWriter writer = facesContext.getResponseWriter();
	...
	protected String getChartVar(StackedBarChart chart) {
		if(chart.getWidgetVar() != null)
			return chart.getWidgetVar();
		else
			return "pf_columnChart_" + chart.getId();
	}
