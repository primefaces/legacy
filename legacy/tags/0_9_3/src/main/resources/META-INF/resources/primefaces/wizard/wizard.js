...
PrimeFaces.widget.Wizard = function(clientId, cfg) {
	this.tabSelector = PrimeFaces.core.Utils.escapeClientId(clientId) + " div.pf-wizard-tab";
	this.clientId = clientId;
	this.cfg = cfg;
	this.step = 0;
	this.navPrev = jQuery(PrimeFaces.core.Utils.escapeClientId(clientId) + "_prev");
	this.navNext = jQuery(PrimeFaces.core.Utils.escapeClientId(clientId) + "_next");
...
PrimeFaces.widget.Wizard.prototype.move = function(stepFactor) {
	...
	params = params + "&" + jQuery(PrimeFaces.core.Utils.escapeClientId(this.cfg.formClientId)).serialize();
	...
	jQuery.ajax({
		...
		success: function(responseXML) {
			...
			if(success == "true") {
				tabToUpdate = PrimeFaces.core.Utils.escapeClientId(wizard.clientId) + "_tab" + stepToGo;
				...
				tabToUpdate = PrimeFaces.core.Utils.escapeClientId(wizard.clientId) + "_tab" + wizard.getStep();
				jQuery(tabToUpdate).html(tabContent);
			}
